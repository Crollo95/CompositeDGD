...
class GaussianMixturePrior(nn.Module):
    ...
    def z_nll_detached_params(self, z: torch.Tensor) -> torch.Tensor:
        """
        Negative log-likelihood of z under the mixture, but with GMM parameters
        treated as constants (detached). This means gradients flow to z only,
        not to means/log_vars/logits.
        """
        # detach parameters so they don't get gradients
        means = self.means.detach().unsqueeze(0)        # (1,K,D)
        log_vars = self.log_vars.detach().unsqueeze(0)  # (1,K,D)
        logits = self.logits.detach()                   # (K,)

        B, D = z.shape
        z_exp = z.unsqueeze(1)                          # (B,1,D)

        inv_vars = torch.exp(-log_vars)                 # (1,K,D)
        diff2 = (z_exp - means) ** 2                    # (B,K,D)

        log_p_zk = -0.5 * (
            D * math.log(2.0 * math.pi)
            + log_vars.sum(dim=-1)                      # (1,K)
            + (diff2 * inv_vars).sum(dim=-1)            # (B,K)
        )                                               # (B,K)

        log_weights = F.log_softmax(logits, dim=0)       # (K,)
        log_mix = log_p_zk + log_weights.unsqueeze(0)    # (B,K)
        log_p = torch.logsumexp(log_mix, dim=1)          # (B,)
...
class SplitRepresentationLayer(nn.Module):
    """
    Two per-sample embedding tables:
      z_i in R^{n_z} always used/learned
      w_i in R^{n_w} used/learned only when cond=1 (baseline cond=0 => w_used=0)
    """
    def __init__(self, n_samples: int, n_z: int, n_w: int, init_std: float = 0.01):
        super().__init__()
        self.n_samples = int(n_samples)
        self.n_z = int(n_z)
        self.n_w = int(n_w)

        self.z = nn.Parameter(torch.randn(n_samples, n_z) * init_std)
        #self.w = nn.Parameter(torch.zeros(n_samples, n_w))
        self.w = nn.Parameter(torch.randn(n_samples, n_w) * init_std)


        
    def forward(self, indices: torch.Tensor, cond_mask: torch.Tensor | None = None):
        ...
        cm = cond_mask.to(z.dtype).view(-1, 1)  # (B,1)
        w_used = w * cm                          # baseline rows -> exact zero and no grad
        return z, w_used
