import numpy as np
from src.metrics_eval import get_latents_split, eval_split
from src.metrics_predictors import cond_pred_train_test, cluster_pred_train_test_mcc

def score_on_validation(model, ds_bundle, device, seed=0, penalty=1.0, centered_auc=True, clf_kind="knn"):
...
def z_cluster_mcc_on_val(model, ds_bundle, device, seed=0, z_cluster_key=None, kind="knn"):
    if z_cluster_key is None:
        z_cluster_key, _ = _infer_cluster_keys(ds_bundle)
    if z_cluster_key is None:
        return 0.0
...
def w_cluster_mcc_on_val(model, ds_bundle, device, seed=0, w_cluster_key=None, kind="knn"):
    if w_cluster_key is None:
        _, w_cluster_key = _infer_cluster_keys(ds_bundle)
    if w_cluster_key is None:
        return 0.0
