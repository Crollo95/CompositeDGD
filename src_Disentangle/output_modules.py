...
class BaseOutputModule(nn.Module):
    ...
    def component_losses(self, y: torch.Tensor, *params: torch.Tensor, **kwargs) -> dict[str, torch.Tensor]:
        """
        Default: a single scalar loss for this module.
        Returns a dict mapping a name to a scalar NLL tensor.
        """
        nll = self.loss(y, *params, **kwargs)
...
class NegativeBinomialOutput_old_(BaseOutputModule):
    """
...
class NegativeBinomialOutput(BaseOutputModule):
    ...
    def __init__(self, in_dim: int, out_dim: int, options: dict | None = None):
        ...
        # --- mean head ---
        self.mean_mode = options.get("mean_mode", "softplus")  # "softplus" or "softmax"
        self.mean_layer = nn.Linear(in_dim, out_dim)

        # --- dispersion ---
        # Paper: r = exp(log_r) + 1 (floor at 1)
        self.dispersion_floor = float(options.get("dispersion_floor", 1.0))  # set to 1.0 to mimic paper (+1)
        log_r_init = float(options.get("log_r_init", 0.0))  # init for log_r (see note below)
        learn_dispersion = bool(options.get("learn_dispersion", True))
        shared_dispersion = bool(options.get("shared_dispersion", False))

        shape = (1, 1) if shared_dispersion else (1, out_dim)
        init = torch.full(shape, log_r_init)
    ...
    def forward(self, h: torch.Tensor):
        logits = self.mean_layer(h)  # (B, F)

        if self.mean_mode == "softmax":
            mean_param = F.softmax(logits, dim=-1)  # proportions (B, F), sum=1
        elif self.mean_mode == "softplus":
            mean_param = F.softplus(logits)         # absolute mean proxy (B, F)
        else:
            raise ValueError(f"Unknown mean_mode={self.mean_mode}")

        return mean_param, self.log_r  # log_r is (1, F) or (1, 1)

    def loss(self, y: torch.Tensor, mean_param: torch.Tensor, log_r: torch.Tensor, scaling_factor: torch.Tensor | None = None):
        """
        If mean_mode == "softmax": mean_param is p, and you MUST provide scaling_factor (or compute it outside).
        If mean_mode == "softplus": mean_param is mu, scaling_factor is ignored.
        """
        eps = 1e-8
        y = y.to(mean_param.dtype)

        if self.mean_mode == "softmax":
            if scaling_factor is None:
                raise ValueError("NegativeBinomialOutput(mean_mode='softmax') requires scaling_factor.")
            # scaling_factor: (B,) or (B,1) -> (B,1)
            sf = scaling_factor.view(-1, 1).to(mean_param.dtype)
            mean = sf * mean_param
        else:
            mean = mean_param

        # dispersion: r > 0 (optionally with floor like paper)
        # log_r is (1,F) or (1,1), broadcast to (B,F)
        log_r = log_r.expand(mean.shape[0], mean.shape[1])
        r = torch.exp(log_r) + self.dispersion_floor
        ...
        log_prob = (
            lgamma(y + r) - lgamma(r) - lgamma(y + 1.0)
            + r * (torch.log(r + eps) - log_r_plus_mu)
            + y * (log_mu - log_r_plus_mu)
        )  # (B, F)
