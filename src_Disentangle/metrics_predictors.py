...
def binary_metrics(train_X, train_y, test_X, test_y, seed=0, kind="logreg", scale=True, balanced=False):
    ...
    acc_fn = balanced_accuracy_score if balanced else accuracy_score
    acc_tr = acc_fn(ytr, yhat_tr)
    acc_te = acc_fn(yte, yhat_te)
...
def cond_pred_train_test(train_X, train_y, test_X, test_y, seed=0, prefix="cond_from_X",
                         kind="logreg", scale=True, balanced=False):
    auc_tr, acc_tr, auc_te, acc_te = binary_metrics(
        train_X, train_y, test_X, test_y, seed=seed, kind=kind, scale=scale, balanced=balanced
    )
    return {
